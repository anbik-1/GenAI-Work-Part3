...
# Directory structure
[[ -f "infrastructure/app.py" ]]  || error "Wrong directory. cd into genese-proposal-ai/ first"
...
cat > /tmp/genese_outputs.env << ENVEOF
...
ENVEOF
info "  All outputs saved to /tmp/genese_outputs.env (source this to reuse variables)"
...
docker build -t genese-api -f services/api/Dockerfile services/ 2>&1 | tail -2
...
docker build -t genese-worker -f services/worker/Dockerfile services/ 2>&1 | tail -2
...
if [[ "$EXIT_CODE" == "0" ]]; then
  ...
  warn "  To check logs: aws logs tail /genese/worker --since 5m --region $REGION"
fi
...
echo -e "  ${YELLOW}Saved outputs:${NC}            source /tmp/genese_outputs.env"
echo ""
